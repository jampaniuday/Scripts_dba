...
declare
   ...
   v_siscoord  varchar2(50):='&SistProj';
...
begin
   ...
   for i in (select sdo_index_owner,table_name,column_name from all_sdo_index_info where sdo_index_owner=v_usuario order by table_name) loop
      ...
      utl_file.put_line(v_file,v_anular||'sdelayer -o register -l '||i.table_name||','||i.column_name||' -e '||v_tipogeom||' -C '||v_nome_id||' -P BASIC -u '||i.sdo_index_owner||' -p '||v_senha||' >> '||v_arq_log);
   ...
   utl_file.put_line(v_file,'echo set serveroutput on size 1000000; > cria_trg.sql');
   utl_file.put_line(v_file,'echo set termout off; >> cria_trg.sql');
   utl_file.put_line(v_file,'echo spool '||v_trg_log||'; >> cria_trg.sql');
   utl_file.put_line(v_file,'echo prompt Criação de Triggers >> cria_trg.sql');
   for j in (select sdo_index_owner,table_name,column_name from all_sdo_index_info where sdo_index_owner=v_usuario order by table_name) loop
      utl_file.put_line(v_file,'echo prompt . >> cria_trg.sql');
      utl_file.put_line(v_file,'echo prompt . Criando Trigger Para '||j.table_name||' >> cria_trg.sql');
      utl_file.put_line(v_file,'echo execute sde.criar_trigger('''||j.sdo_index_owner||''','''||j.table_name||''','''||j.column_name||'''); >> cria_trg.sql');
   end loop;
   utl_file.put_line(v_file,'echo spool off; >> cria_trg.sql');
   utl_file.put_line(v_file,'echo exit >> cria_trg.sql');
   
   utl_file.put_line(v_file,'sqlplus '||v_usuario||'/'||v_senha||' @cria_trg.sql');
