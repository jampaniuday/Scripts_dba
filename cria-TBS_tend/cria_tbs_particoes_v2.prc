...
begin
 ...
 /* execute immediate 'CREATE TABLESPACE SGR_PART_AUDITORIA_' || mes_ano ||
                    ' DATAFILE   ''+DATA/uat/datafile/sgr_part_auditoria_' ||
                    lower(mes_ano) ||
                    '.dbf''
SIZE 11M AUTOEXTEND ON NEXT 32K MAXSIZE UNLIMITED LOGGING ONLINE PERMANENT EXTENT MANAGEMENT LOCAL AUTOALLOCATE BLOCKSIZE 32K SEGMENT SPACE MANAGEMENT AUTO FLASHBACK ON';

  execute immediate 'CREATE TABLESPACE SGRPART_AUDITORIA_ITEM_' || mes_ano ||
                    ' DATAFILE   ''+DATA/uat/datafile/sgrpart_auditoria_item_' ||
                    mes_ano ||
                    '.dbf''
SIZE 11M AUTOEXTEND ON NEXT 32K MAXSIZE UNLIMITED LOGGING ONLINE PERMANENT EXTENT MANAGEMENT LOCAL AUTOALLOCATE BLOCKSIZE 32K SEGMENT SPACE MANAGEMENT AUTO FLASHBACK ON';
...
PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT) TABLESPACE "PART_LIM_SAL_BAIXA_A16"';
  ...
  execute immediate 'alter table ADM_SGR_AUD.auditoria add partition ' ||
                    mes_ano || ' VALUES LESS THAN
(TO_DATE(''20' || SUBSTR(mes_ano, 2, 2) || '-' ||
                    SUBSTR(mes, 6, 2) ||
                    '-01 00:00:00'', ''SYYYY-MM-DD HH24:MI:SS'', ''NLS_CALENDAR=GREGORIAN''))
 SEGMENT CREATION IMMEDIATE PCTFREE 10 PCTUSED 40 INITRANS 1 MAXTRANS 255  NOCOMPRESS NOLOGGING STORAGE(INITIAL 8388608 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS UNLIMITED
 PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT)  TABLESPACE "SGR_PART_AUDITORIA_' ||
                    mes_ano || '"';

  execute immediate 'alter table ADM_SGr_AUD.auditoria_item add partition ' ||
                    mes_ano || ' VALUES LESS THAN
(TO_DATE(''20' || SUBSTR(mes_ano, 2, 2) || '-' ||
                    SUBSTR(mes, 6, 2) ||
                    '-01 00:00:00'', ''SYYYY-MM-DD HH24:MI:SS'', ''NLS_CALENDAR=GREGORIAN''))
SEGMENT CREATION IMMEDIATE PCTFREE 10 PCTUSED 40 INITRANS 1 MAXTRANS 255 NOCOMPRESS NOLOGGING STORAGE(INITIAL 8388608 NEXT 1048576 MINEXTENTS 1 MAXEXTENTS UNLIMITED
PCTINCREASE 0 FREELISTS 1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT) TABLESPACE "SGRPART_AUDITORIA_ITEM_' ||
                    mes_ano || '"';
